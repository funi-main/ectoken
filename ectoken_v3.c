...
int encrypt(unsigned char* ao_ciphertext, int* ao_ciphertext_len,
            unsigned char* ao_tag, const unsigned char* a_plaintext,
            const int a_plaintext_len, const unsigned char* a_key,
            const unsigned char* a_iv, const int a_iv_len)
{
...
int decrypt(unsigned char* ao_plaintext, int* a_plaintext_len,
            const unsigned char* a_ciphertext, const int a_ciphertext_len,
            ...
            const int a_tag_len)
{
         ...
         */
        if(!EVP_DecryptUpdate(l_ctx, ao_plaintext, &l_len, a_ciphertext,
                              a_ciphertext_len))
        {
                l_ret = -6;
                goto fail;
                          ...
                          const char* a_key, const int a_key_len)
{
        ...
        l_ret = decrypt(l_token, &l_token_len, l_ciphertext, l_ciphertext_len,
                        (unsigned char*)l_key, l_iv, G_IV_LEN, l_tag, G_TAG_LEN);
        switch (l_ret)
        {
                case 0:
                        break;
                          ...
                          const char* a_key, const int a_key_len)
{
        ...
        int l_ciphertext_len = a_plaintext_len;
        unsigned char l_ciphertext[l_ciphertext_len];
        unsigned char l_iv[G_IV_LEN];
        unsigned char l_tag[G_TAG_LEN];
        unsigned char l_key[G_KEY_LEN];
        ...
        l_ret = encrypt(l_ciphertext, &l_ciphertext_len, l_tag,
                        (unsigned char*) a_plaintext, a_plaintext_len,
                        (unsigned char*) l_key, l_iv, G_IV_LEN);
